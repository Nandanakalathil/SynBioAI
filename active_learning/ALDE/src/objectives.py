...
class Production(Objective):
    ...
    def __init__(self, df, name, encoding, obj_col):
        ...
        if encoding == 'onehot':
            ...
            from src.encoding_utils import generate_esm2
            self.Xtrain = generate_esm2(train_combos, batch_size=2048)
            self.X = torch.load('data/' + name + '/ESM2_x.pt')
        else:
            raise ValueError(f"Encoding {encoding} not supported in Production class yet")
