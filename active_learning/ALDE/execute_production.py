...
if __name__ == "__main__":
    ...
    parser.add_argument("--encoding", type=str, default="ESM2")
    ...
    parser.add_argument("--mtype", type=str, default="DKL_BOTORCH", help="Specific model type to run. If None, runs all.")
    parser.add_argument("--acq_fn", type=str, default="TS", help="Specific acquisition function to run. If None, runs all.")
    ...
    for r in range(index, index + runs):
        ...
        kernel=args.kernel
        mtypes_to_run = ["BOOSTING_ENSEMBLE", "GP_BOTORCH", "DNN_ENSEMBLE", "DKL_BOTORCH"] if args.mtype == "None" else [args.mtype]
        acq_fns_to_run = ['GREEDY', 'UCB', 'TS'] if args.acq_fn == "None" else [args.acq_fn]
        
        for mtype in mtypes_to_run: 
            for acq_fn in acq_fns_to_run: 
